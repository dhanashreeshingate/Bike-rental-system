...
def carlist(request):
    vehicles = Vehicles.objects.all().order_by('-PricePerDay')
...
def signup(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            UserDetails.objects.create(user=user, ContactNo=ContactNo, dob=dob, Country=Country, City=City, Address=Address)
            error = "no"
        except:
            error = "yes"
...
def addVehicle(request):
    ...
    if request.method == "POST":
        ...
        if 'ShutterLock' in request.POST:
            ShutterLock = request.POST['ShutterLock']
        else:
            ShutterLock = "no"

        if 'ExternalFuelFilling' in request.POST:
            ExternalFuelFilling = request.POST['ExternalFuelFilling']
        else:
            ExternalFuelFilling = "no"

        if 'PassSwitch' in request.POST:
            PassSwitch = request.POST['PassSwitch']
        else:
            PassSwitch = "no"

        if 'LEDTailLights' in request.POST:
            LEDTailLights = request.POST['LEDTailLights']
        else:
            LEDTailLights = "no"

        if 'LowFuelIndicator' in request.POST:
            LowFuelIndicator = request.POST['LowFuelIndicator']
        else:
            LowFuelIndicator = "no"

        if 'PilotLamps' in request.POST:
            PilotLamps = request.POST['PilotLamps']
        else:
            PilotLamps = "no"

        if 'RadialTyre' in request.POST:
            RadialTyre = request.POST['RadialTyre']
        else:
            RadialTyre = "no"

        if 'DiscBrake' in request.POST:
            DiscBrake = request.POST['DiscBrake']
        else:
            DiscBrake = "no"
        ...
        try:
            Vehicles.objects.create(VehiclesBrand=brandsid, VehiclesTitle=VehiclesTitle,
                                    ...
                                    FuelType=FuelType, ModelYear=ModelYear, Vimage1=Vimage1,
                                    Vimage2=Vimage2, Vimage3=Vimage3, Vimage4=Vimage4, Vimage5=Vimage5, ShutterLock=ShutterLock,
                                    ExternalFuelFilling=ExternalFuelFilling, PassSwitch=PassSwitch, LEDTailLights=LEDTailLights,
                                    LowFuelIndicator=LowFuelIndicator, PilotLamps=PilotLamps, RadialTyre=RadialTyre, DiscBrake=DiscBrake)
            error = "no"
        except:
            error = "yes"
...
def editVehicle(request,pid):
    ...
    if request.method == "POST":
        ...
        vehicle.ShutterLock = ShutterLock
        vehicle.ExternalFuelFilling = ExternalFuelFilling
        vehicle.PassSwitch = PassSwitch
        vehicle.LEDTailLights = LEDTailLights
        vehicle.LowFuelIndicator = LowFuelIndicator
        vehicle.PilotLamps = PilotLamps
        vehicle.RadialTyre = RadialTyre
        vehicle.DiscBrake = DiscBrake
...
def vehicleDetails(request,pid):
    ...
    if request.method == "POST":
        ...
        try:
            Booking.objects.create(user=userdtls, VehicleId=vehicle, FromDate=FromDate, ToDate=ToDate, message=message, BookingNumber=bookingNo)
            error = "no"
        except:
            error = "yes"
